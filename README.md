...
- **通用**：开机自启、应用快捷键行为（按住查看 / 双按隐藏 / 循环窗口 / 只打开）、单击 Caps Lock 的行为、调试日志。
...
| `repeatPress` | 应用快捷键的行为：`"peek"`（按住 Hyper+字母时显示，松开字母或 Hyper 后隐藏并切回原应用）、`"hide"`（再次按同一字母时隐藏，默认）、`"cycle"`（重复按时循环该应用的窗口）、`"none"`（只打开）。旧配置里的 `toggleHideIfFrontmost` 仍然认（`true`→`hide`，`false`→`none`），保存时会写成新键。 |
