...
- **一眼看得出是什么**：每行按类型排版——文本是 `Aa`，代码和路径是 `❯` 加等宽绿字，链接是首字母图标加域名标签，文件是带扩展名的彩色标牌（PSD 蓝、PDF 红、ZIP 琥珀），颜色就是那块颜色本身。列表按 收藏 / 今天 / 昨天 / 本周 / 更早 分组；过滤页带条数徽标；筛选行右侧显示 **↩ 会粘贴到哪个应用**。
- **连续的图片折叠成网格**：三张以上挨在一起的图片会排成 3 列缩略图，`⌘` 拖拽可以像在访达里一样框选多张，被选中的按粘贴顺序编号。
- **深浅两副面孔**：面板头部的 ☾ / ☀ 随时切换，默认跟随系统，选过之后记在配置里。
- **认得出内容**：邮箱 / 电话 / 文件路径 / JSON / 代码 会被识别，代码和路径用等宽字体显示。
- **颜色是一等公民**：复制的颜色显示为色块，预览里给出 HEX / RGB / HSL 三种写法，各自一键复制。
- **粘贴为…**：右键任何文本条目，可以转大写 / 小写 / 首字母大写 / 去首尾空白 / 合并空行 / 压成单行后粘贴。
- **拖拽双向**：条目能拖进任何应用；反过来把文本、图片、文件**拖进面板**就直接存为历史。
- **编辑后再粘**：右键「编辑…」改完正文，可以直接保存并粘贴。
- **删错了能撤销**：删除后按 `⌘Z` 恢复。
- **队列看得见**：⇥ 切到「队列」页，按粘贴顺序列出所有已收集条目，可移出、上移、下移。队列存盘，重启不丢。
- **行能读得完**：文本条目在列表里按正文长度显示 1 / 2 / 3 行——一行的话就显示一行，两行显示两行，超过三行才截断。
- **预览是一张卡片**：悬停就出现在列表旁边，高度按内容裁——图片按自己的比例，文本按自己的长度——底部一行写清来源、大小、时间和 `⌘C`。按 `→` 可以让它跟着键盘走；富文本按原样式渲染，图片可直接在预览程序打开。**列表里已经显示完的文本（三行以内）不弹卡片**：没截断就没有可预览的东西；图片、链接、文件、颜色照旧，卡片才是它们完整的样子。
- **图片能就地放大**：指针落在预览图上，滚滚轮或双指开合就放大，**放的是指针底下那一块**——想把哪处细节看清楚，指哪放大哪，不用先放大再找。放大后**拖动图片**即可按当前倍数查看其他位置（放大时显示「倍数 · 拖动查看」，光标变成手形）。双击复位；指针移出图片、或换到别的条目，自动回到适应窗口大小。
- **预览卡片放哪边都够得着**：卡片会按屏幕空间放在列表左或右。**指针停在某一行上，卡片才跟过去**（约 90ms）；连续划过列表时卡片不动，所以横穿列表去够另一侧的卡片不会被中途改掉目标，而且往哪个方向走都一样快。高亮始终立刻跟随指针。
- **收藏可排序**：收藏页里拖拽调整顺序，顺序永久保存。
- **面板随你**：大小三档（紧凑/标准/宽大）、打开位置（**鼠标右侧**（默认）/ 屏幕中央 / 鼠标正下方 / 屏幕底部居中）、`↩` 的默认动作（粘贴或仅复制）都在设置里；深浅外观在面板头部直接切。
- 头部四个小按钮：切换外观、高级语法、保存筛选、`?` 快捷键速查表。多选之后头部出现「已选 N · ↩ 合并粘贴」。
...
鼠标侧：`⌘点击` 连续粘贴（粘完不关面板）、`⌥点击` 多选、`⌘拖拽` 在图片网格里框选、悬停唤出预览、右键有完整操作菜单，行还可以直接拖出去。预览图上：滚轮或双指开合**以指针位置放大**，**拖动平移**，双击复位。

### 行该有多高

改版前每行是「34pt 方图标 + 标题 + 副标题」，副标题写的是「ChatGPT · 15 小时前」。它整齐，但整页都在重复同两个事实，而且一屏只装得下十来条。

那两个事实搬进了悬停预览卡片——**只在被看的那一条上说一次**——行省下的高度让同样一屏装得下差不多两倍。省下来的注意力换成了别的东西：行首那块标记现在真的在区分类型，不再是一个统一的灰方块。

但一行装不下段落。现在文本条目按正文自己的长度显示 1 / 2 / 3 行：一句话就是一行，一句话分两行就显示两行，超过三行才截断——于是**列表本身就是可读的**，不用逐条悬停去猜内容。卡片随之只管它真正擅长的那部分：三行没说完的正文，以及缩略图、链接、文件、颜色这些一行本来就说不到底的东西。没被截断的文本悬停不开卡片，因为那三行已经在眼前了。

图片是另一类收益最大的。十张截图排成十条几乎一样的灰条，不悬停根本认不出哪张是哪张；排成 3 列缩略图就一眼可辨，`⌘` 框选也才有地方落脚。少于一整行（三张）的图片仍然按行显示——三分之二空着的网格比两条图片行说得更少。

底部那条常驻提示栏一起去掉了。它在任何列表下都说同样三件事，占掉一整条列表的高度；真正需要说的「刚才那下成功了」，现在由粘贴完浮出来的吐司在**它为真的那一刻**说，而且只在面板不关的那几种操作里说。快捷键全表还在，`?` 一按就有。
