...
- **图片能就地放大**：指针落在预览图上，滚滚轮或双指开合就放大，**放的是指针底下那一块**——想把哪处细节看清楚，指哪放大哪，不用先放大再找。双击复位；指针移出图片、或换到别的条目，自动回到适应窗口大小。放大时左上角显示倍数。
...
鼠标侧：`⌘点击` 连续粘贴（粘完不关面板）、`⌥点击` 多选、`⌘拖拽` 在图片网格里框选、悬停唤出预览、右键有完整操作菜单，行还可以直接拖出去。预览图上：滚轮或双指开合**以指针位置放大**，双击复位。
