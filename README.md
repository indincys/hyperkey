...
- **通用**：开机自启、应用快捷键行为（按住查看 / 再按返回 / 循环窗口 / 只打开）、单击 Caps Lock 的行为、调试日志。
...
| `repeatPress` | 应用快捷键的行为：`"peek"`（按住 Hyper+字母时显示，松开字母或 Hyper 后隐藏并切回原应用）、`"hide"`（再次按同一字母时返回打开前的应用，默认；目标应用允许时同时隐藏）、`"cycle"`（重复按时循环该应用的窗口）、`"none"`（只打开）。旧配置里的 `toggleHideIfFrontmost` 仍然认（`true`→`hide`，`false`→`none`），保存时会写成新键。 |
...
- **来源可见**：每行显示复制来源应用的图标；列表按 收藏 / 今天 / 昨天 / 本周 / 更早 分组；过滤页带条数徽标；面板头部显示 **↩ 会粘贴到哪个应用**。
- **认得出内容**：邮箱 / 电话 / 文件路径 / JSON / 代码 会被识别并打上徽标，代码和路径用等宽字体显示。
...
- **预览不靠鼠标**：按 `→` 锁定预览窗格跟随键盘选择；富文本条目按原样式渲染，图片可直接在预览程序打开。
- **收藏可排序**：收藏页里拖拽调整顺序，顺序永久保存。
- **面板随你**：大小三档（紧凑/标准/宽大）、打开位置（居中/鼠标处/屏幕底部）、`↩` 的默认动作（粘贴或仅复制）都在设置里。
- 按 `?` 显示完整快捷键速查表；`⌘A` 全选后底部出现批量操作栏。
