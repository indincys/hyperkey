...
- **预览是一张卡片**：悬停就出现在列表旁边，高度按内容裁——图片按自己的比例，文本按自己的长度——底部一行写清来源、大小、时间和 `⌘C`。按 `→` 可以让它跟着键盘走；富文本按原样式渲染，图片可直接在预览程序打开。
...
现在这两个事实搬进了悬停预览卡片——**只在被看的那一条上说一次**——行本身压成一行，同样高度装得下差不多两倍。省下来的注意力换成了别的东西：行首那块标记现在真的在区分类型，不再是一个统一的灰方块。列表里没找到的，鼠标停一下就有。

图片是这次改动收益最大的一类。十张截图排成十条几乎一样的灰条，不悬停根本认不出哪张是哪张；排成 3 列缩略图就一眼可辨，`⌘` 框选也才有地方落脚。少于一整行（三张）的图片仍然按行显示——三分之二空着的网格比两条图片行说得更少。
